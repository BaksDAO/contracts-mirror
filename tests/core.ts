...
import { percent, usd } from "./shared/helpers";
...
describe("Core", () => {
  ...
  describe("exchange fund", () => {
    it("updates minimum liquidity", async () => {
      const minimumLiquidity = await env.core.minimumLiquidity();
      const newMinimumLiquidity = usd(60000);

      await expect(env.core.setMinimumLiquidity(newMinimumLiquidity))
        .to.emit(env.core, "MinimumLiquidityUpdated")
        .withArgs(minimumLiquidity, newMinimumLiquidity);

      expect(await env.core.minimumLiquidity()).to.be.equal(
        newMinimumLiquidity,
      );
    });

    it("updates servicing threshold", async () => {
      const servicingThreshold = await env.core.servicingThreshold();
      const newServicingThreshold = percent(1.5);

      await expect(env.core.setServicingThreshold(newServicingThreshold))
        .to.emit(env.core, "ServicingThresholdUpdated")
        .withArgs(servicingThreshold, newServicingThreshold);

      expect(await env.core.servicingThreshold()).to.be.equal(
        newServicingThreshold,
      );
    });
  });
});
